...
def get_superpixels(image):
    ...
    it returns a label array
    """
    h, w = image.shape[:2]
    print("Generating superpixels...")

    # Cityscapes
    retval= cv2.ximgproc.createSuperpixelSEEDS(w, h, 3, 2400, num_levels=3, prior=2, histogram_bins=5)
    retval.iterate(image, 50)
    superpixels = retval.getLabels()
    print("Number of superpixels: {}".format(np.max(superpixels)))
...
def add_scribble(graph, scribble, label):
    """
